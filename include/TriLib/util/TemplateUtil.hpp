...
#define ARITHMETIC_TEMPLATE(x) template<typename x, typename = std::enable_if<std::is_arithmetic<x>::value || std::negation<std::bool_constant<std::is_const<x>::value>>::value>>
#define SSE_REGISTER_TEMPLATE(x) template<typename x, typename = std::enable_if<std::is_same<tl::detail::__m64_wrapper, x>::value || std::is_same<tl::detail::__m128_wrapper, x>::value || std::is_same<tl::detail::__m128d_wrapper, x>::value || std::is_same<tl::detail::__m128i_wrapper, x>::value>>
namespace tl::templates{
    template<bool A, bool B>
    using OR = std::bool_constant<std::disjunction<A, B>::value>;
}
#elif __cplusplus == 202002L
#define ARITHMETIC_TEMPLATE(x) template<typename x> requires std::is_floating_point<x>::value ||std::integral_constant<bool, !std::is_const<x>::value>::value>
#define SSE_REGISTER_TEMPLATE(x) template<typename x> requires std::is_same<tl::detail::__m64_wrapper, x>::value || std::is_same<tl::detail::__m128_wrapper, x>::value || std::is_same<tl::detail::__m128d_wrapper, x>::value || std::is_same<tl::detail::__m128i_wrapper, x>::value
namespace tl::templates{
    template<bool A, bool B>
    using OR = std::bool_constant<std::disjunction<A, B>::value>;
}
